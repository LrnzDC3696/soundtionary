...
// const AGORA_CHANNEL = sessionStorage.getItem('room')
...
let joinAndDisplayLocalStream = async () => {
    document.getElementById('room-name').innerText = AGORA_CHANNEL

    client.on('user-published', handleUserJoined)
    client.on('user-left', handleUserLeft)

    try{
        UID = await client.join(AGORA_APP_ID, AGORA_CHANNEL, AGORA_TOKEN, UID)
    }catch(error){
        console.error(error)
        window.open('/', '_self')
    }
...
let createMember = async () => {
    let response = await fetch('/create_member/', {
        ...
        body:JSON.stringify({'name':NAME, 'room_name':AGORA_CHANNEL, 'UID':UID})
    })
...
let getMember = async (user) => {
    let response = await fetch(`/get_member/?UID=${user.uid}&room_name=${AGORA_CHANNEL}`)
    let member = await response.json()
    return member
}
